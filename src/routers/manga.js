...
router.get('/manga/detail/:slug', async (req, res) => {
  ...
  try {
    const response = await AxiosService(`${baseUrl}/manga/${slug}/`)
    const $ = cheerio.load(response.data)
    const element = $('.perapih')
    let genre_list = []
    let chapter = []
...
router.get('/search/:query', async (req, res) => {
  ...
  try {
    const response = await AxiosService(
      `${baseUrl}/cari/?post_type=manga&s=${query}`
    )
    const $ = cheerio.load(response.data)
...
router.get('/manga/popular/:pagenumber', async (req, res) => {
  ...
  try {
    const response = await AxiosService(`${baseUrl}/${url}`)
...
router.get('/recommended', async (req, res) => {
  try {
    const response = await AxiosService(`${baseUrl}/other/rekomendasi/`)
